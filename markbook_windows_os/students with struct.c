...
void enumer(char *filepath)
{
    ...
    {
        fseek(f,i,SEEK_SET);            ///êàòî ñòèãíå íà êàðÿ ïàê ìó êàçâà äà ÷åòå, çàòîâà ñòàâà áåçêðàåí öèêúë
        if(fread(&s,sizeof(s),1,f))
        {
            s.No=j;
            j++;
...
void new_lessons(char *path_copy)
{
    ...
    do
    {
        ...
        i=repeat(path_copy,predmet);           ///ïðîâåðêà äàëè å ïîâå÷å îò åäèí
    }while(1);
    fclose(f);
    sort_in_file(path_copy);                          ///àêî ñå ïîâòàðÿ, îáðúùà íàîáðàòíî
}
...
void new_classes(char *mainpath)
{
    ...
    {printf("There is such a class\n"); new_classes(mainpath);}  /// ïðîâåðè äàëè ñúùåñòâóâà íàïð: 12á = 12 á = 12Á = 12 Á
